...
(function() {
    ...
    baidu.object.extend(LuShu.prototype, {
        ...
        */
        ...
        _moveNext: function (index) { 
            ...
            if (index < this._path.length - 1) {
                ...
                if (difftime > 600) {//大于600秒就画出静止点 
                    ...
                    var showText = []; //"";
                    showText.push("<div style=\"line-height:5px;\"> <p class='p'>停留:" + stopDate + "</p>");
                    showText.push("<p class='p'>开始:" + startTime + " </p>"); 
                    showText.push("<p class='p'>结束:" + endTime + "</p>");
                    showText.push("<p class='p'>经纬度:" + me._path[index].lng + "," + me._path[index].lat + "</p>");
                    //showText.push("<p class='p'>地址:<span name=\"tableAddress" + rowid + "\"></span></p>");
                    showText.push("</div>");
                    var infoWindow = new BMap.InfoWindow(showText.join(''), opts);  // 创建信息窗口对象
                ...
                me._opts.defaultContent = "<div style=\"text-align:left;line-height:5px;\"><p><strong>" + deviceName + "</strong> </p>" + "<p>时间：" + showInfoWindowData[index].time + "<p><p>经纬度：" + me._path[index].lat + "," + me._path[index].lng + "<p>"
                    + "<p>速度：" + showInfoWindowData[index].speed + ",方向：" + showInfoWindowData[index].Course + "</p> <div>"; //" + MeterToKilometer(distance) + "
             
            }
            if (index == this._path.length - 1)  plus.nativeUI.toast("播放结束.");;
        },
        /**
        ...
        _getAddress: function (lat, lon, id, idOrName) {
            $.ajax({
                url: "/AjaxService/DeviceMonitor.asmx/GetAddress", 
                data: '{ "lat": '+lat+', "lon": '+lon+', "mapType": "BAIDU", language :""}',
               // dataType: "text", 
                success: function (address) { 
                    if (idOrName == undefined || idOrName == "id") { 
                        if (id != undefined) $("#" + id).text(address.d);
                    } else {
                        $("[name=" + id + "]").text(address.d);
                    }
                }
            });
        }
